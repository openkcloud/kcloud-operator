...
없음. 기존 NVIDIA/Furiosa/RNGD/Rebellions DS는 영향을 받지 않습니다.
...
```bash
helm rollback npu-operator 1 -n npu-operator
```
...
### 변경 내용
  ...
  operator에서 직접 관리되도록 개선되었습니다.
- **chart version**: `0.4.7` → `0.5.0` (minor bump — TT device plugin 통합)
...
# 3. Rollout 완료 대기
kubectl rollout status deploy/npu-operator-controller-manager -n npu-operator

# 4. 검증 — 5 벤더 DS READY 확인
kubectl -n kube-system get ds | grep npu-op

# 5. Regression — allocatable 자원 확인
```

### Rollback

```bash
helm rollback npu-operator 0 -n npu-operator
```
...
```bash
# 1. 구 device plugin DaemonSet 5종 삭제 (orchestrated cleanup)
...
# 신 이름 DS 생성 확인 (5 expected if all vendors enabled)
...
## v0.5.10 → v0.5.11 (chart 0.5.10 → 0.5.11, 2026-05-14)
